...
bool Adafruit_FXOS8700::begin(fxos8700AccelRange_t rng) {
  ...
  write8(FXOS8700_REGISTER_CTRL_REG1, 0x05);

  /* Configure the magnetometer */
  /* Hybrid Mode, Over Sampling Rate = 4 */
...
bool Adafruit_FXOS8700::getRawData(int16_t *data)
{
    /* Read 13 bytes from the sensor */
  Wire.beginTransmission((byte)FXOS8700_ADDRESS);
#if ARDUINO >= 100
...
/* ToDo: Check status first! */
  ...
  /* Shift values to create properly formed integer, this is the raw untransformed data */
  *(data)     = (int16_t)((axhi << 8) | axlo) >> 2;
  *(data + 1) = (int16_t)((ayhi << 8) | aylo) >> 2;
  *(data + 2) = (int16_t)((azhi << 8) | azlo) >> 2;
  *(data + 6) = (int16_t)((mxhi << 8) | mxlo);
  *(data + 7) = (int16_t)((myhi << 8) | mylo);
  *(data + 8) = (int16_t)((mzhi << 8) | mzlo);
