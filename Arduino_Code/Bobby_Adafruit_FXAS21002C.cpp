...
bool Adafruit_FXAS21002C::begin(gyroRange_t rng) {
  ...
  */
  ...
  write8(GYRO_REGISTER_CTRL_REG1, 0x06);     // Active
...
/* Same as the above function but only gets the raw values for easier transfer via BLE */
bool Adafruit_FXAS21002C::getRawData(int16_t *data)
{
  /* Read 7 bytes from the sensor */
  Wire.beginTransmission((byte)FXAS21002C_ADDRESS);
#if ARDUINO >= 100
  ...
  *(data + 3) = (int16_t)((xhi << 8) | xlo);
  *(data + 4) = (int16_t)((yhi << 8) | ylo);
  *(data + 5) = (int16_t)((zhi << 8) | zlo);
